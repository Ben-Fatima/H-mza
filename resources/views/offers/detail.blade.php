<x-layout>
  <div class="w-11/12 mx-auto">
      <x-title
          >Welcome
          <p class="inline orange">{{$offer->product->user->username}}</p>
          , we wish you good luck!</x-title
      >
      <div class="flex mt-8">
          <div class="w-1/4">
              <img
                  src="/storage/{{$offer->product->user->profile_picture}}"
                  class="rounded-full w-9/12 mx-auto"
              />
              <p class="text-2xl font-bold text-center py-2">
                  {{$offer->product->user->username}}
              </p>
              <p class="py-2 px-8 text-justify">{{$offer->product->user->about}}</p>
              <a href="/profile/edit">
                  <button
                      class="
                          mx-8
              ...
              <p class="px-8 py-2">
                  ...
                  >{{$offer->product->user->address}}
              </p>
              <p class="px-8 py-2">
                  <i class="fas fa-envelope-open-text px-1"></i>
                  {{$offer->product->user->email}}
              </p>
          </div>
          <div class="flex-grow">
              ...
              <div class="pb-4 grid grid-cols-1 mx-auto mt-4">
                  @foreach($offer->product->user->products as $product)
                  <div
                  class="
